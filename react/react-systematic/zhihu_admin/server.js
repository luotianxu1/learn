...
app.use(async (req, res, next) => {
    ...
    if (!token) {
        responsePublic(res, false, {
            codeText:
                'no permission to access, possibly because you are not logged in or your login has expired',
        })
        return
    }
    // 校验签名的合法性
...
app.post('/store', async (req, res) => {
    ...
    if (
        store.some(
            (item) => item.userId == req.$TOKEN.id && item.newsId == newsId
        )
    ) {
        responsePublic(res, false, {
...
app.post('/phone_code', async (req, res) => {
    ...
    if (item) {
        ...
        codeData.push({
            id:
                codeData.length === 0
                    ? 1
                    : +codeData[codeData.length - 1].id + 1,
            phone,
            code,
            time: +new Date(),
        })
    ...
    // 发送短信
    await fs.appendFile(
        `${path.resolve(__dirname, '')}/code.txt`,
        `手机号 ${phone} 的用户于 ${new Date(time).toLocaleString('zh-CN', {
            hour12: false,
        })} 发送短信验证码：${code} \n`,
        'utf-8'
    )
...
app.post('/login', async (req, res) => {
    ...
    if (!flag) {
        ...
        await fs.writeFile(
            `${pathdb}/code.json`,
            JSON.stringify(req.$CODEDATA),
            'utf-8'
        )
        responsePublic(res, false, {
            codeText:
                'the mobile phone number and verification code do not match',
        })
        return
    }
    ...
    if (!item) {
        item = {
            id:
                req.$USERDATA.length === 0
                    ? 1
                    : +req.$USERDATA[req.$USERDATA.length - 1].id + 1,
